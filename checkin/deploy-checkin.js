...
const SA_KEY = path.join(__dirname, 'checkinaura-4cde2-firebase-adminsdk-fbsvc-689b7e655d.json');
const SITE   = 'checkinaura-4cde2';
const FILES  = [
  'checkin.html',
  'checkin-sw.js',
  'checkin-manifest.json',
  'checkin-icon.svg',
  'checkin-version.json',
];
...
async function main() {
  ...
  const ver = await api('POST', `${base}/sites/${SITE}/versions`, token, {
    config: {
      rewrites: [{ glob: '**', path: '/checkin.html' }],
      headers: [
        { glob: '/checkin.html',         headers: { 'Cache-Control': 'no-cache, no-store, must-revalidate' } },
        { glob: '/checkin-version.json', headers: { 'Cache-Control': 'no-cache, no-store, must-revalidate' } },
      ],
    },
  });
